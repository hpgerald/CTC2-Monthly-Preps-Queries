...
INTO LA 
FROM srt_apt 
WHERE (((srt_apt.Given)<#7/1/2020#) AND ((srt_apt.Appointment)<#2/1/2021#)) 
...
INTO LE 
FROM srt_tst 
WHERE (((srt_tst.DateInitiatedEAC)<#7/1/2020#)) 
...
INTO LT 
FROM srt_tst 
WHERE (((srt_tst.TestDate)<#7/1/2020#)) 
GROUP BY srt_tst.PatientID 
...
INTO LS 
FROM srt_sts 
WHERE (((srt_sts.Status) Is Not Null) AND ((srt_sts.StatusDate)<#7/1/2020#)) 
...
INTO LV 
FROM srt_vst 
WHERE (((srt_vst.NumDaysDispensed)>0) AND ((srt_vst.VisitDate)<#7/1/2020#)) 
...
INTO FIPT 
FROM srt_vst 
WHERE (((srt_vst.VisitDate) Between #11/1/2019# And #11/30/2019#) AND ((srt_vst.TBRXIPTID)="START IPT")) 
ORDER BY srt_vst.VisitDate; 
```
...
DateDiff("yyyy",[DoB],44012) AS Age, IIf([Gender]="Male","M","F") AS Sex, LV.[Last ART Visit] AS [ART Visit],
...
IIf(([Dispensed]<200 And [TX_CURR]>=44012),1,0) AS CURR_JUN, 
...
AS [File Namba], DateDiff("yyyy",[DoB],44012) AS Age, IIf([Gender]="Male","M","F") AS Sex, LV.[Last ART Visit]
...
AS LTFU_v0_Doc, [TX_CURR]+1 AS LTFU_v1_Adj, IIf(([Dispensed]<200 And [TX_CURR]>=44012),1,0) AS CURR_JUN, 
LS.[Last Status], LS.[Status Date], LS.Reason, LS.Notes AS [Status Notes], LT.[Test Date], LT.[Result Date], 
LT.Results, LT.Notes AS [VRL Notes], IIf([Test Date]>43646 And [Test Date]<44013,1,0) AS Sample, 
IIf([Sample]=1 And [Results]>0,1,0) AS Result, IIf([Gender]="Female" And [CURR_JUN]=1 And [Age]>=15 And [Age] <=49,1,0) 
...
WHERE (((srt_vst.VisitDate) Between #6/1/2020# And #6/30/2020#) AND ((srt_vst.ARVStatusCode)=2)) 
