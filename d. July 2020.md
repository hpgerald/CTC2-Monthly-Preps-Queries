...
INTO LA 
FROM srt_apt 
WHERE (((srt_apt.Given)<#8/1/2020#) AND ((srt_apt.Appointment)<#3/1/2021#)) 
...
INTO LE 
FROM srt_tst 
WHERE (((srt_tst.DateInitiatedEAC)<#8/1/2020#)) 
...
INTO LT 
FROM srt_tst 
WHERE (((srt_tst.TestDate)<#8/1/2020#)) 
GROUP BY srt_tst.PatientID 
...
INTO LS 
FROM srt_sts 
WHERE (((srt_sts.Status) Is Not Null) AND ((srt_sts.StatusDate)<#8/1/2020#)) 
...
INTO LV 
FROM srt_vst 
WHERE (((srt_vst.NumDaysDispensed)>0) AND ((srt_vst.VisitDate)<#8/1/2020#)) 
...
INTO FIPT 
FROM srt_vst 
WHERE (((srt_vst.VisitDate) Between #12/1/2019# And #12/31/2019#) AND ((srt_vst.TBRXIPTID)="START IPT")) 
ORDER BY srt_vst.VisitDate; 
```
...
DateDiff("yyyy",[DoB],44043) AS Age, IIf([Gender]="Male","M","F") AS Sex, LV.[Last ART Visit] AS [ART Visit],
...
IIf(([Dispensed]<200 And [TX_CURR]>=44043),1,0) AS CURR_JUL, 
...
AS [File Namba], DateDiff("yyyy",[DoB],44043) AS Age, IIf([Gender]="Male","M","F") AS Sex, LV.[Last ART Visit]
...
AS LTFU_v0_Doc, [TX_CURR]+1 AS LTFU_v1_Adj, IIf(([Dispensed]<200 And [TX_CURR]>=44043),1,0) AS CURR_JUL, 
...
IIf([Sample]=1 And [Results]>0,1,0) AS Result, IIf([Gender]="Female" And [CURR_JUL]=1 And [Age]>=15 And [Age] <=49,1,0) 
