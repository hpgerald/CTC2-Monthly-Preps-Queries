...
```sql
SELECT DISTINCT 
PatientID, StatusDate, Status, StatusReason, TransferredOutWhere 
INTO srt_sts 
FROM tblStatus 
...
```sql
SELECT DISTINCT 
PatientID, TestDate, ResultDate, ResultNumeric, ResultReturnDate, 
...
INTO LA 
FROM srt_apt 
WHERE (((srt_apt.Given)<#4/1/2021#) AND ((srt_apt.Appointment)<#12/1/2021#)) 
...
INTO LE 
FROM srt_tst 
WHERE (((srt_tst.DateInitiatedEAC)<#4/1/2021#)) 
...
INTO LT 
FROM srt_tst 
WHERE (((srt_tst.TestDate)<#4/1/2021#)) 
GROUP BY srt_tst.PatientID 
...
Last(srt_sts.StatusReason) AS Reason 
INTO LS 
FROM srt_sts 
WHERE (((srt_sts.Status) Is Not Null) AND ((srt_sts.StatusDate)<#4/1/2021#)) 
...
INTO LV 
FROM srt_vst 
WHERE (((srt_vst.NumDaysDispensed)>0) AND ((srt_vst.VisitDate)<#4/1/2021#)) 
...
INTO FER 
FROM srt_vst 
WHERE (((srt_vst.VisitDate) Between #2/1/2021# And #2/28/2021#) AND ((srt_vst.ARVStatusCode)=2)) 
...
INTO FIPT 
FROM srt_vst 
WHERE (((srt_vst.VisitDate) Between #8/1/2020# And #8/31/2020#) AND ((srt_vst.TBRXIPTID)="START IPT")) 
ORDER BY srt_vst.VisitDate; 
```
...
DateDiff("yyyy",[DoB],44286) AS Age, IIf([Gender]="Male","M","F") AS Sex, LV.[Last ART Visit] AS [ART Visit],
...
IIf(([Dispensed]<200 And [TX_CURR]>=44286),1,0) AS CURR_MAR, 
...
AS [File Namba], DateDiff("yyyy",[DoB],44286) AS Age, IIf([Gender]="Male","M","F") AS Sex, LV.[Last ART Visit]
...
AS LTFU_v0_Doc, [TX_CURR]+1 AS LTFU_v1_Adj, IIf(([Dispensed]<200 And [TX_CURR]>=44286),1,0) AS CURR_MAR, 
LS.[Last Status], LS.[Status Date], LS.Reason, LT.[Test Date], LT.[Result Date], 
LT.Results, IIf([Test Date]>43921 And [Test Date]<44287,1,0) AS Sample, 
IIf([Sample]=1 And [Results]>0,1,0) AS Result, IIf([Gender]="Female" And [CURR_MAR]=1 And [Age]>=15 And [Age] <=49,1,0) 
...
##### 18: IPT COMPLETION FOR THE REPORTING PERIOD (MONTH)
```sql
...
##### 19: EARLY RETENTION FOR THE REPORTING PERIOD (MONTH)
```sql
...
##### 20: SDI FOR THE REPORTING PERIOD (QUARTER)
```sql
...
WHERE (((srt_vst.VisitDate) Between #1/1/2021# And #3/31/2021#) AND ((srt_vst.ARVStatusCode)=2)) 
